    ...
    try {
      if (tab === 'streamers') {
        ...
        const list = ((d||[]) as any[]).map((p,i)=>({
          ...
          score:Math.floor(Math.random()*9900)+100,
    ...
    <AppLayout>
      <div className="max-w-xl mx-auto px-4 py-5">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background:'rgba(0,229,255,0.08)', border:'1px solid rgba(0,229,255,0.15)' }}>
        ...
        <div className="flex gap-1.5 mb-6 overflow-x-auto no-scrollbar">
          {tabs.map(([t,l]) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ background: tab===t?'#1a56db':'#1a1a1a', color: tab===t?'#000':'#888' }}
            >
                    ...
                    >
                      ...
                      <div className="relative">
                        <Avatar className={`${isFirst?'w-16 h-16':'w-12 h-12'} rounded-full ring-2`}
                          style={{ '--tw-ring-color': isFirst?'#1a56db':'#333' } as any}>
                          <AvatarImage src={u.avatar_url||''}/>
                          <AvatarFallback style={{background:'#1a1a1a',color:'#1a56db',fontWeight:800}}>{u.display_name[0]?.toUpperCase()}</AvatarFallback>
                        </Avatar>
                        {u.is_live && (
                      ...
                      <div className={`w-14 ${podiumH} rounded-t-lg flex flex-col items-center justify-end pb-2`}
                        style={{ background: isFirst?'rgba(0,229,255,0.12)':'#1a1a1a', border:`1px solid ${isFirst?'rgba(0,229,255,0.3)':'#222'}` }}
                      >
                        <p className="text-xs font-bold" style={{ color:isFirst?'#1a56db':'#888' }}>{u.score.toLocaleString()}</p>
                      </div>
                    </motion.div>
                  );
                })}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold" style={{color:'#1a56db'}}>{u.score.toLocaleString()}</p>
