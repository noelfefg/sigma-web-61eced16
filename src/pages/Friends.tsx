      ...
      <div className="max-w-xl mx-auto px-4 py-5">
        ...
        <div className="flex gap-1 mb-5 p-1 rounded-xl" style={{ background:'#111' }}>
          {(['discover','friends','requests'] as Tab[]).map(t => (
            ...
            >{t}{t==='friends' && followingIds.size>0 && <span className="ml-1 px-1 py-0.5 rounded text-[9px]" style={{ background:'#1a56db', color:'#000' }}>{followingIds.size}</span>}</button>
          ))}
        </div>
          ...
          <motion.div key={tab} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
            {loading ? (
              ...
              <div className="text-center py-14">
                <Bell className="w-10 h-10 mx-auto mb-3 opacity-20" style={{ color:'#1a56db' }}/>
                <p style={{ color:'#555' }}>No pending requests</p>
              </div>
            ) : filtered.length === 0 ? (
              <div className="text-center py-14">
                <LordIcon icon={ICONS.users} size={18} trigger="hover" primary="1a56db" className="w-10 h-10 mx-auto mb-3 opacity-20" style={{ color:'#1a56db' }}/>
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      {followingIds.has(u.id) ? (
                        <>
                          <Link to="/messages">
                            <button className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[#1a1a1a] transition-colors" title="Message">
                              <MessageSquare className="w-3.5 h-3.5" style={{ color:'#1a56db' }}/>
                            </button>
                          </Link>
                        ...
                        <button onClick={() => addFriend(u.id)}
                          ...
                          style={{ background:'#1a56db', color:'#000' }}
                        ><LordIcon icon={ICONS.plus} size={18} trigger="click" primary="1a56db" className="w-3 h-3"/>Add</button>
                      )}
                    </div>
                  </motion.div>
                ))}
