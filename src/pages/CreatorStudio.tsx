  ...
  if (!user) return (
    <AppLayout>
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <Radio className="w-12 h-12 opacity-20" style={{color:'#6b7280'}}/>
      ...
      <div className="max-w-5xl mx-auto px-4 py-5">
        ...
        <div className="flex items-center gap-4 mb-6 p-4 rounded-2xl" style={{background:'#111',border:'1px solid #1e1e1e'}}>
          <Avatar className="w-14 h-14 rounded-xl">
            ...
            <AvatarFallback style={{background:'#1a1a1a',color:'#6b7280',fontWeight:800,fontSize:20}}>{profile?.display_name?.[0]?.toUpperCase()}</AvatarFallback>
          </Avatar>
        ...
        <div className="flex gap-1 mb-6 p-1 rounded-xl overflow-x-auto no-scrollbar" style={{background:'#111'}}>
          {tabs.map(([t,l,icon]) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ background: tab===t?'#1a1a1a':'transparent', color: tab===t?'#6b7280':'#555' }}
            >{icon}{l}</button>
          ))}
        </div>
        ...
        {tab === 'dashboard' && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                {label:'Total Views',val:'0',icon:<LordIcon icon={ICONS.eye} size={18} trigger="hover" primary="6b7280" className="w-4 h-4"/>,col:'#6b7280'},
                {label:'Followers',val:'0',icon:<LordIcon icon={ICONS.users} size={18} trigger="hover" primary="6b7280" className="w-4 h-4"/>,col:'#22c55e'},
            ...
            <div className="p-4 rounded-2xl" style={{background:'#111',border:'1px solid #1e1e1e'}}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  ...
                  {label:'Friends',icon:<LordIcon icon={ICONS.users} size={18} trigger="hover" primary="6b7280" className="w-4 h-4"/>,path:'/friends'},
                  ...
                  <Link key={a.label} to={a.path}>
                    <button className="w-full flex flex-col items-center gap-2 p-3 rounded-xl transition-colors hover:opacity-90"
                      style={{
                        background: a.accent?'rgba(100,100,100,0.1)':'#1a1a1a',
                        color: a.accent?'#6b7280':'#888',
                        border:`1px solid ${a.accent?'rgba(100,100,100,0.2)':'#222'}`,
                      }}
            ...
            <div className="rounded-2xl overflow-hidden" style={{background:'#111',border:'1px solid #1e1e1e'}}>
              ...
              {streams.length === 0 ? (
                <div className="py-12 text-center">
                  <Radio className="w-10 h-10 mx-auto mb-3 opacity-20" style={{color:'#6b7280'}}/>
                ...
                <div key={s.id} className="flex items-center gap-3 px-4 py-3" style={{borderBottom:'1px solid #1a1a1a'}}>
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{background:'#1a1a1a'}}>
                    <Play className="w-4 h-4" style={{color:'#6b7280'}}/>
                  </div>
        ...
        {tab === 'content' && (
          <div className="py-12 text-center rounded-2xl" style={{background:'#111',border:'1px solid #1e1e1e'}}>
            <Video className="w-10 h-10 mx-auto mb-3 opacity-20" style={{color:'#6b7280'}}/>
        ...
        {tab === 'analytics' && (
          <div className="py-12 text-center rounded-2xl" style={{background:'#111',border:'1px solid #1e1e1e'}}>
            <LordIcon icon={ICONS.trending} size={22} trigger="loop" primary="22c55e"  className="w-10 h-10 mx-auto mb-3 opacity-20" style={{color:'#6b7280'}}/>
